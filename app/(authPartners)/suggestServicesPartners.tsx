...
const SuggestServicesPartners: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const cadastrarServicoRecusado = async (servico: any, motivo: string) => {
    try {
      ...
      setServicos((prevState) => prevState.filter((item) => item.id !== servico.id)); 
...
export default SuggestServicesPartners;