...
const partnerDetail: React.FC<{}> = () => {
  ...
  const [diasSemanaClinica, setDiasSemanaClinica] = useState<string[]>([]);
  ...
  const [turnoClinica, setTurnoClinica] = useState<string[]>([]);
  ...
  const handleSelectDiaSemana = (diaSemanaClinica: string) => {
    setDiasSemanaClinica(prevState => {
      if (prevState.includes(diaSemanaClinica)) {
        return prevState.filter(item => item !== diaSemanaClinica);
      } else {
        return [...prevState, diaSemanaClinica];
      }
    });
  };

  const handleSelectTurno = (turnoClinica: string) => {
    setTurnoClinica(prevState => {
      if (prevState.includes(turnoClinica)) {
        return prevState.filter(item => item !== turnoClinica);
      } else {
        return [...prevState, turnoClinica];
      }
    });
  };
    ...
    const buscarDados = async () => {
      try {
        ...
        if (diasPreferenciaSnap.exists()) {
          setDiasSemanaClinica(diasPreferenciaSnap.data().diasSemana || []);
        }
        ...
        if (turnoPreferenciaSnap.exists()) {
          setTurnoClinica(turnoPreferenciaSnap.data().turno || []);
        }
        ...
        <>
          ...
          {step === 2 && (
            <View style={styles.section}>
              ...
              <TextInput style={styles.input} placeholder="Estado" value={dados.estado || ""} onChangeText={(text) => setDados({ ...dados, estadoClinica: text })} />
              <TextInput style={styles.input} placeholder="Cidade" value={dados.cidade || ""} onChangeText={(text) => setDados({ ...dados, cidadeClinica: text })} />
              <TextInput style={styles.input} placeholder="Bairro" value={dados.bairro || ""} onChangeText={(text) => setDados({ ...dados, bairroClinica: text })} />
              <TextInput style={styles.input} placeholder="Rua" value={dados.rua || ""} onChangeText={(text) => setDados({ ...dados, ruaClinica: text })} />
              <TextInput style={styles.input} placeholder="Número" value={dados.numero || ""} onChangeText={(text) => setDados({ ...dados, numeroClinica: text })} />
              ...
              <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity
                      style={[
                        styles.specialityItem,
                        diasSemanaClinica.includes(item) && styles.selectedSpeciality,
                      ]}
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={[
                      styles.specialityItem,
                      turnoClinica.includes(item) && styles.selectedSpeciality,
                    ]}
